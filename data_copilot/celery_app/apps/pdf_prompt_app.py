...
def from_url(self, url: str, extra_info: Optional[Dict] = None) -> List[Document]:
    ...
    return self.load_data(temp_file_path, extra_info)


# This line adds the from_url method to the PDFReader class
PDFReader.from_url = from_url
...
def process_pdf(url: str, prompt: str):
    if url.startswith("http://") or url.startswith("https://"):
        pdfs = PDFReader().from_url(url)
    else:
        pdfs = PDFReader().load_data(Path(url))
    
    index = GPTVectorStoreIndex.from_documents(pdfs)
    query_engine = index.as_query_engine()
    result =  query_engine.query(prompt)
    if result.response:
        answer = result.response.strip("\n ")
    else:
        answer = "Your document does not contain any text"
    return answer
...
def translate_user_prompt(
    ...
    try:
        ...
        if file_type == "pdf":
            answer = process_pdf(sas_url, user_prompt)

        else:
            answer = f"Wrong file type: {file_type} only pdf is supported"
            logging.error(
                f"Wrong file type: {file_type} only pdf is supported --"
